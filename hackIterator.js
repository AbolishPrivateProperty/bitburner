...
export async function main(ns) {
	...
	if (hsThreads < 1) {
		nthreads2h = 1;
		hsThreads = 1;
		iterations = Math.round((avsram / hackram));
	}
	...
	while (true) {
		...
		let moneyDeficit = (ns.getServerMaxMoney(serverName) - ns.getServerMoneyAvailable(serverName)) / ns.getServerMaxMoney(serverName);
		if (securityDeficit < 5 && moneyDeficit < 0.05) {
